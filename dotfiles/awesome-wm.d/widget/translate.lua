...
local BASE_URL = 'https://translate.googleapis.com/translate_a/single?client=gtx&sl=%s&tl=%s&dt=t&q=%s'
...
local function translate(to_translate, lang)
    local sl = lang:sub(1, 2)
    local tl = lang:sub(4)
    local urll = string.format(BASE_URL, sl, tl, urlencode(to_translate))

    local resp_json, code = https.request(urll)
    if (code == 200 and resp_json ~= nil) then
        local resp = json.decode(resp_json)[1][1][1]
